...
7. **Battery arbitrage revenue now rests on a handful of scarcity hours.** Measured
   15 Sep 2026, build `2026-09-15a`, defaults. `bessBenchmark` on `r.marginalP`
   returns 2,135,964 R/MW/yr at 4 hours. The price series is flat coal for most of the
   year — median R750, p90 R759 — with a scarcity tail:

   ```
   price cap applied      arbitrage R/MW/yr
   none                          2,135,964
   R6,205  (p99)                 1,709,087
   R2,000                          351,040
   R1,500                          189,560
   R1,000                           33,357
   ```

   Six hours above R20,000, topping at R87,000, carry R426,877 — **20% of annual
   arbitrage revenue from 0.07% of the hours**. This is the scarcity pricing added on
   11–12 Sep, working as designed, not a defect. But it means the arbitrage figure
   inherits the whole uncertainty of the VoRS curve, which is an ADMINISTRATIVE
   ASSUMPTION and not a market outcome. Two consequences:

   - It explains the 23x divergence that superseded the battery saturation table in
     RESULTS.md. That entry's note "arbitrage is held flat in this model" is no longer
     true and the table has not been reissued.
   - Any battery revenue figure quoted externally must state the VoRS shape it rests
     on, or it is a number about an assumption rather than about South Africa.

   OPEN: whether to reissue the saturation table on the current basis. That is a
