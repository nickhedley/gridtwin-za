...
regional = {
    ...
    "meta": {
        ...
        "HYDRA_CENTRAL_SPLIT": (
            "CORRECTED 29 Aug 2026. This key was called HYDRA_CENTRAL_ZERO and said Hydra "
            "Central was set to zero. THAT HAS NOT BEEN TRUE SINCE 14 Aug 2026: it carries "
            "669 MW of wind and 459.5 MW of solar, applied by step 1b of this script from "
            "nodal/supply_area_split_draft.json. The stale text was dangerous rather than "
            "merely wrong - it invited a regeneration with the older copy of this script, "
            "which has no step 1b and silently reverts the split while updating its own "
            "fingerprint so the damage looks consistent. "
            "WHAT HYDRA CENTRAL IS: a transmission supply area spanning the Karoo across the "
            "Northern Cape / Eastern Cape provincial boundary (Hydra, Gamma, Koruson, Aquila). "
            "The IPP Office reports by PROVINCE, so the split is derived by apportioning each "
            "province's online figure using contracted shares from named projects - applied as "
            "SHARES, never as absolute MW, so every technology total is preserved. "
            "STILL OUTSTANDING: roughly 73 MW of BW5 solar absent from the published location "
            "table, possibly Graspan. See pending_next_quarterly in the split draft. "
            "CAUTION on extending the method: nearest-substation matching was FALSIFIED at "
            "Impofu on 28 Aug 2026 - it returned Grassridge at 106.9 km when the line actually "
            "built runs 116 km to Chatty, which sits NEARER at 93.3 km and was simply absent "
            "from substations_compact.json. The method is only as good as the substation "
