...
// 36/36 while it still reported green.
{
  ...
  const _miss = ['Deep decarbonisation 2035'].filter(n => !_have.has(n));
  results.push({ name: 'every preset this harness uses exists', actual: _miss.length ? 'missing: ' + _miss.join(', ') : 'all present', expected: 'all present', tol: null, pass: _miss.length === 0, source: 'Guard against silently skipped checks', unit: '' });
}
(async () => {
  const dom = boot();
  ...
  if (cost && !cost.error && cost.base && cost.base.avgCost) {
    ...
    check('avgCost has not moved without a decision', cost.base.avgCost, 1059.1, 10.59,
      ...
      + 'that total to 99.9% on that fleet. Until then NO system-cost aggregate carried any '
  ...
  const future = pressPreset('Deep decarbonisation 2035');
  if (future) {
